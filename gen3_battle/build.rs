...
fn main() {
    ...
    let mut out = String::from("// @generated by gen3_battle/build.rs — do not edit.\n\n");
...
fn emit_species(
    ...
    for m in &species {
        ...
        if let Some(ls) = learnsets
            .get(id)
            .and_then(|e| e.get("learnset"))
            .and_then(|v| v.as_object())
        {
            let mut rows: Vec<(usize, u64)> = Vec::new();
            for (move_id, sources) in ls {
                let Some(idx) = move_index(move_id) else {
                    continue;
                };
    ...
    out.push_str("\n];\n\n");
    println!(
        "cargo:warning=gen3_battle: {} learnset rows",
        learn_flat.len()
    );
...
fn emit_randbat(randbats: &Value, move_ids: &[String], out: &mut String) {
    ...
    for (display, entry) in obj {
        ...
        let Some(roles) = entry.get("roles").and_then(|v| v.as_object()) else {
            continue;
        };
        for role in roles.values() {
            let Some(list) = role.get("moves").and_then(|v| v.as_array()) else {
                continue;
            };
            let start = flat.len();
            for m in list {
    ...
    out.push_str("];\n\n");
    println!(
        "cargo:warning=gen3_battle: {} randbat sets, {dropped} moves not in the gen3 table",
        sets.len()
    );
}
...
fn emit_moves(dump: &Value, out: &mut String) -> Vec<String> {
    ...
    for m in &moves {
        ...
        let reflectable = m["reflectable"].as_bool().unwrap_or(false);
        let snatchable = m["snatchable"].as_bool().unwrap_or(false);
        let status_action = match m["statusAction"].as_object() {
            ...
            Some(act) => {
                if let Some(status) = act.get("status").and_then(|v| v.as_str()) {
                    ...
                    String::from("Some(StatusAction::Snatch)")
        ...
        out.push_str(&format!(
            "    MoveEntry {{ id: {id:?}, name: {name:?}, move_type: {}, \
             ...
             reflectable: {reflectable}, snatchable: {snatchable}, \
             self_drop: {self_drop} }},\n",
