...
# Pico body (top side) + USB tab
ax.add_patch(FancyBboxPatch((X(S.PC0-0.5),Y(S.PTOP-0.7)),X(20),Y(S.PBOT-S.PTOP+1.4),
             boxstyle="round,pad=0.01,rounding_size=0.03",fc="#464650",ec="#28282d",lw=1.3,alpha=0.92,zorder=2))
ax.text(X(S.PC0+10),Y((S.PTOP+S.PBOT)/2),"Raspberry Pi Pico  (mounted on TOP, USB left)",
        ha="center",va="center",fontsize=8,fontweight="bold",color="#eee",zorder=3)

# modules (OLED screens, LED connectors, MB102, USB tab)
for (nm,c0,h0,c1,h1) in S.MODULES:
    ax.add_patch(FancyBboxPatch((X(c0),Y(h0)),X(c1-c0),Y(h1-h0),
                 boxstyle="round,pad=0.005,rounding_size=0.02",fc="#232838",ec="#11141f",lw=1.0,alpha=0.85,zorder=2.6))
    if nm.startswith('OLED'):
        ax.add_patch(Rectangle((X(c0+0.7),Y(h0+3.0)),X(c1-c0-1.4),Y(h1-h0-4.6),fc="#0a1a3a",ec="#3af",lw=0.8,zorder=2.7))
        ax.text(X((c0+c1)/2),Y((h0+h1)/2+0.8),nm.split(' (')[0],ha="center",va="center",
                fontsize=7,color="#7cf",fontweight="bold",zorder=2.8)
# switch bodies
for (c0,h0,c1,h1) in S.SW_BODIES:
    ax.add_patch(FancyBboxPatch((X(c0)-0.03,Y(h0)),X(c1-c0)+0.06,Y(h1-h0),
                 boxstyle="round,pad=0.004,rounding_size=0.02",fc="#3a3a3a",ec="#111",lw=0.8,alpha=0.9,zorder=2.5))

# jumpers (underside wires)
for j in S.JUMPERS:
    (c1,h1),(c2,h2) = j['a'], j['b']
    ax.plot([X(c1),X(c2)],[Y(h1),Y(h2)],color=rgbf(S.netrgb(j['net'])),lw=1.3,zorder=5,alpha=0.9)
# cuts
for (c,h) in S.CUTS:
    ax.plot([X(c)-0.028,X(c)+0.028],[Y(h)-0.028,Y(h)+0.028],color="red",lw=1.5,zorder=7)
    ax.plot([X(c)-0.028,X(c)+0.028],[Y(h)+0.028,Y(h)-0.028],color="red",lw=1.5,zorder=7)
# pads
for p in S.PADS:
