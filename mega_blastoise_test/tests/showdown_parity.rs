...
fn gen3_turns_match_showdown() {
    ...
    for (c, got) in cases.iter().zip(&results) {
        ...
        let script = TurnScript {
            seats: [
                Some(SeatScript { hit: c.script[0].0, crit: c.script[0].1, random: c.script[0].2, secondary: false, immobile: false, hits: 0 }),
                Some(SeatScript { hit: c.script[1].0, crit: c.script[1].1, random: c.script[1].2, secondary: false, immobile: false, hits: 0 }),
            ],
        };
        let (ours_hp, ours_pp) = {
