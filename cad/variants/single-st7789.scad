// Mega Blastoise — enclosure  ::  VARIANT: single-st7789
// ONE shared central ST7789 240x240 colour TFT (replaces the two mono
// OLEDs). Each player gets a button block + LEDs flanking the screen on
// their half; buzzer relocated beside the display. Colour enables sprites.
// NOTE: a shared central screen is read upright by one player only — this
// variant suits a Stadium-style shared "battle view", not private text.
//
...
// ST7789 1.54" 240x240 IPS colour TFT, SPI. Nominal — caliper-verify.
disp    = [32.0, 33.0, 4.5];
disp_active = [27.7, 27.7];
disp_lip_t  = 1.0;                          // front bezel lip
disp_conn   = 5.0;                          // pin-header relief, one edge
pmod    = [34.0, 22.0, 7.0];                // MH-CD42 charge+boost+protect
lipo    = [50.0, 34.0, 5.5];
slide_sw   = [8.7, 3.6, 3.5];
buzzer_d   = 12.0;  buzzer_h = 9.0;
btn        = 12.0;  btn_pitch = 15.24;      // grid-locked 6 * 2.54
btn_hole_d = 4.5;
hp_led_d     = 5.0;
...
// distances from board centre (display at y=0), per player half
...
function yc(p, o) = p * o;                   // p=-1 / +1, o = dist from centre
...
bz_x = disp[0]/2 + buzzer_d/2 + 3;           // buzzer beside the display
...
assert(bz_x - buzzer_d/2 - 1.5 > disp[0]/2,
       "buzzer overlaps the display — increase bz_x");
...
assert(faceplate_t > led_diffuser_t && faceplate_t > disp_lip_t,
       "faceplate too thin for diffuser skin / display lip");
assert(boost_bumpout || interior_depth >= pmod[2] + clearance,
       "power module won't fit");

echo(VARIANT="single-st7789", board_w=board_w, board_l=board_l,
     total_h=total_h, half_l=half_l, y_led=y_led, y_move=y_move, y_sw=y_sw);
...
module button_centres() {
  for (p=[-1,1]) {
    for (i=[0:3]) translate([(i-1.5)*btn_pitch, yc(p,y_move)]) children();  // move
    for (i=[-1:1]) translate([i*btn_pitch,     yc(p,y_sw)])  children();    // switch
  }
}
...
module bottom_tub() {
  ...
  // buzzer locating collar, beside the display
...
module top_faceplate() {
  difference() {
    ...
    // central display — module seats from tub side, front bezel lip remains
    translate([0,0,-eps])
    ...
    // pin-header relief: deepen the underside pocket along one edge
    translate([0, -(disp[1]/2), -eps])
      linear_extrude(faceplate_t - disp_lip_t + eps)
        square([disp[0]+2*clearance, disp_conn], center=true);
    // buttons (4 move + 3 switch per player)
    button_centres()
    ...
    // buzzer grille beside the display
    translate([bz_x,0,0]) {
...
module ghosts() {
  ...
  color("purple",0.6) translate([-rp2040[0]/2, -board_l/2+end_margin+lipo[0]+4,
                                  floor_t]) cube(rp2040);
  ...
  color("blue",0.4)   translate([-disp[0]/2,-disp[1]/2,tub_h])
                         cube([disp[0],disp[1],1]);     // shared TFT
